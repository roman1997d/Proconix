...
| operative_dashboard.html + operative_dashboard.js | Dashboard operativ | Login; me, work-hours (GPS), project/current, **tasks** (legacy + Planning), issues, uploads; **Log Work**: `GET/POST /api/operatives/work-log`, upload, **Time Sheet Jobs Builder** + perioadă obligatorie, generare PDF via **`POST /timesheet/generate`** (PDFKit pe server, fallback jsPDF), **Create Work Report** (modal) cu încărcare PDF generat; **`timesheetJobs`** + **`priceWorkJobs`** (QA price work, `stepPhotoUrls`) + `photoUrls` la submit; **Price work booking** modal (`GET /qa/assigned-jobs`, încărcare poze `/work-log/upload`); **Archive** pe intrări în „My work entries” (`POST .../work-log/:id/archive`). **Modal task**: poze confirmare, acțiuni Decline / Mark in progress / Complete — detaliu: [10-price-work-booking-qa-photos.md](10-price-work-booking-qa-photos.md) |
...
| operative_dashboard.js | POST login; GET me; work-hours/*; GET project/current; tasks + PATCH + photos; issues; uploads; **GET work-log**; **GET qa/assigned-jobs**; **POST work-log/upload**; **POST work-log** (inclusiv `timesheetJobs`, **`priceWorkJobs`**, `photoUrls`); **POST work-log/:id/send-invoice-copy**; **POST work-log/:id/archive**; **POST timesheet/generate**; **POST work-report/generate** (headers sesiune operativ) |
