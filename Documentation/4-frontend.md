...
| proconix_administration.html + proconix_administration_dashboard.js | Consolă admin | `GET /api/platform-admin/me` cu header-e `X-Platform-Admin-Id` / `Email`; fără sesiune validă → login; secțiuni: companies, platform users, **Billing & plans** (`GET /api/platform-admin/billing-subscriptions`), System & health, etc. |
...
| operative_dashboard.html + operative_dashboard.js | Dashboard operativ | Login operative; /api/operatives/me, work-hours (Clock In/Out cu geolocație), project/current, **tasks** (legacy `tasks` + Task & Planning unde `assigned_to` conține numele operativului), issues, uploads, work-log; mesaje dacă operativul nu este „on site” la pontaj. **Modal task**: click pe rând → detalii, **Confirmation photos (n / 10)**, upload imagini (max 10), acțiuni **Decline** / **Mark in progress** / **Complete** (texte UI în engleză) |
...
| operative_dashboard.js | POST /api/operatives/login (sau login-temp); GET /api/operatives/me; POST work-hours/clock-in, clock-out (GPS); GET work-hours/status, weekly; GET project/current; GET /api/operatives/tasks (listă combinată); GET /api/operatives/tasks/:taskId?source= (detalii + `confirmation_photos`); PATCH /api/operatives/tasks/:taskId (body: `source`, `action`: decline \| in_progress \| complete); POST /api/operatives/tasks/:taskId/photos (multipart, `source`); POST issues (multipart); POST uploads (multipart); GET work-log; POST work-log/upload; POST work-log |
