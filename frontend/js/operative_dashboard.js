...
(function () {
  ...
  function generateAndUploadWorkReport() {
    ...
    function formatWorkersLineForTimesheetPdf() {
      var primary = getOperativeName();
      var parts = [];
      var seen = {};
      function push(n) {
        var s = String(n || '').trim();
      ...
      push(primary);
      (pendingWorklogCollaborators || []).forEach(function (c) {
        push(c && c.name);
      });
      return parts.join(', ');
    }
    ...
    function generateTimesheetHtmlFallbackBlob() {
      ...
      var jobBlocks = jobs
        .map(function (j, idx) {
          ...
          var paths = []
            .concat(Array.isArray(j.photoPaths) ? j.photoPaths : [])
            .concat(Array.isArray(j.existingPhotoPaths) ? j.existingPhotoPaths : [])
            .map(function (x) {
              return String(x || '').trim();
            })
            .filter(Boolean);
          function absImgSrc(u) {
            if (!u) return '';
            if (/^https?:\/\//i.test(u) || /^data:image\//i.test(u)) return u;
            if (/^uploads\//i.test(u)) u = '/' + u.replace(/^\/+/, '');
            if (u.indexOf('/') === 0 && typeof window !== 'undefined' && window.location && window.location.origin) {
    ...
    return uploadJobPhotosToServer().then(function () {
      ...
      var payloadJobsMeta = jobs.map(function (j) {
        var photoList = []
          ...
          .filter(Boolean);
        var seenP = {};
        photoList = photoList.filter(function (p) {
          var k = p.toLowerCase();
          if (seenP[k]) return false;
          seenP[k] = true;
          return true;
        });
        return {
      ...
      var payloadMeta = {
        ...
        period_to: periodTo,
        workerName: worker,
        workersDisplay: formatWorkersLineForTimesheetPdf(),
        collaboratorsDisplay: (pendingWorklogCollaborators || []).map(function (c) {
