...
function formatDateTime(v) {
  ...
  return `${formatDateShort(d)} ${hh}:${mi}`;
}
...
function attachPdfFooter(doc) {
  let drawingFooter = false;
  const drawFooter = () => {
    if (drawingFooter) return;
    drawingFooter = true;
    const pageWidth = doc.page.width - doc.page.margins.left - doc.page.margins.right;
    // Keep footer inside printable area to avoid implicit page creation loops.
    const y = doc.page.height - doc.page.margins.bottom - 10;
    try {
      doc.save();
      doc.fontSize(9).fillColor('#6b7280')
        .text('Generated by proconix.uk', doc.page.margins.left, y, {
          width: pageWidth,
          align: 'center',
          lineBreak: false,
        });
...
function photoSourceToDataUrl(src) {
  const s = String(src || '').trim();
  if (!s) return null;
  if (s.startsWith('data:image/')) return s;
  const data = bufferFromPhotoSource(s);
  if (!data || !data.buffer) return null;
  return `data:${data.mime || 'image/jpeg'};base64,${data.buffer.toString('base64')}`;
}
...
function buildDailyRecordsSummaryPdf(records, meta) {
  return new Promise((resolve, reject) => {
    ...
    doc.on('error', reject);
    attachPdfFooter(doc);
    doc.fontSize(20).fillColor('#0f172a').text('Daily Records Report');
    doc.moveTo(doc.page.margins.left, 64).lineTo(doc.page.width - doc.page.margins.right, 64).strokeColor('#cbd5e1').stroke();
    doc.moveDown(0.5);
    doc.fontSize(11).fillColor('#111827').text(`Operative: ${meta.workerName}`);
    doc.text(`Project: ${meta.projectName}`);
    doc.text(`Period: ${formatPeriodLabel(meta.fromDate, meta.toDate)}`);
    doc.text(`Generated: ${formatDateTime(new Date())}`);
    doc.moveDown(0.8);
    records.forEach((r, idx) => {
      doc.fontSize(12).fillColor('#0f172a').text(`${idx + 1}. ${r.stage} - ${r.unitName}`, { underline: false });
      doc.fontSize(10).fillColor('#334155').text(`Status: ${r.status} | Date: ${formatDateTime(r.date)}`);
...
function buildDailyRecordsFullPdf(records, meta) {
  return new Promise((resolve, reject) => {
    ...
    doc.on('error', reject);
    attachPdfFooter(doc);
    doc.fontSize(20).fillColor('#0f172a').text('Daily Records Full Report');
    doc.moveTo(doc.page.margins.left, 64).lineTo(doc.page.width - doc.page.margins.right, 64).strokeColor('#cbd5e1').stroke();
    doc.moveDown(0.5);
    doc.fontSize(11).fillColor('#111827').text(`Operative: ${meta.workerName}`);
    doc.text(`Project: ${meta.projectName}`);
    ...
    records.forEach((r, idx) => {
      ...
      doc.fontSize(14).fillColor('#0f172a').text(`${r.stage} - ${r.unitName}`);
      doc.fontSize(10).fillColor('#334155').text(`Status: ${r.status}`);
      doc.text(`Date: ${formatDateTime(r.date)}`);
      if (r.reason) doc.text(`Reason: ${r.reason}`);
      doc.moveDown(0.3);
      doc.fontSize(11).fillColor('#111827').text(`Comment: ${r.comment || '-'}`);
      doc.moveDown(0.5);
      r.photos.forEach((p, i) => {
        const data = bufferFromPhotoSource(p.src);
        if (!data) return;
        try {
          doc.fontSize(9).fillColor('#334155').text(`Photo ${i + 1}: ${p.name || 'photo'}`);
...
function buildUnitReportPdf(unitName, records, meta) {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(18).fillColor('#0f172a').text('Unit Report');
    doc.moveTo(doc.page.margins.left, 62).lineTo(doc.page.width - doc.page.margins.right, 62).strokeColor('#cbd5e1').stroke();
    doc.moveDown(0.5);
    doc.fontSize(11).fillColor('#111827').text(`Unit: ${unitName}`);
    doc.text(`Operative: ${meta.workerName}`);
    doc.text(`Project: ${meta.projectName}`);
    doc.text(`Period: ${formatPeriodLabel(meta.fromDate, meta.toDate)}`);
    doc.moveDown(0.8);
    doc.fontSize(12).fillColor('#0f172a').text('Jobs details');
    doc.moveDown(0.4);
    ...
    const sorted = Array.isArray(records)
      ...
      : [];
    if (!sorted.length) {
      doc.fontSize(10).fillColor('#334155').text('- No records in selected period.');
    } else {
      sorted.forEach((r) => {
        doc.fontSize(10).fillColor('#334155').text(`${formatDateTime(r.date)} -> ${r.comment || '-'}`);
      });
    }
...
function buildZipIndexPdf(unitsSummary, meta) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ margin: 36, size: 'A4' });
    const chunks = [];
    doc.on('data', (c) => chunks.push(c));
    ...
    doc.fontSize(18).fillColor('#0f172a').text('Daily Records ZIP Index');
    doc.moveTo(doc.page.margins.left, 62).lineTo(doc.page.width - doc.page.margins.right, 62).strokeColor('#cbd5e1').stroke();
    doc.moveDown(0.5);
    doc.fontSize(11).fillColor('#111827').text(`Operative: ${meta.workerName}`);
    doc.text(`Project: ${meta.projectName}`);
    doc.text(`Period: ${formatPeriodLabel(meta.fromDate, meta.toDate)}`);
    doc.moveDown(0.8);
    doc.fontSize(12).fillColor('#0f172a').text('Units summary');
    doc.moveDown(0.4);

    if (!Array.isArray(unitsSummary) || !unitsSummary.length) {
      doc.fontSize(10).fillColor('#334155').text('- No units included.');
    } else {
      unitsSummary.forEach((u, idx) => {
        doc.fontSize(10).fillColor('#334155').text(
          `${idx + 1}. ${u.unitName} | Entries: ${u.entriesCount} | Photos: ${u.photosCount}`
        );
      });
    }
...
function buildDailyRecordsFullHtml(records, meta) {
  const period = `${formatDateShort(meta.fromDate)} - ${formatDateShort(meta.toDate)}`;
  ...
  const items = sorted.map((r, idx) => {
    ...
    const photosHtml = photos.map((p) => {
      const src = photoSourceToDataUrl(p.src);
      if (!src) return '';
      const name = escapeHtml(p.name || 'photo');
      return `<figure><img src="${src}" alt="${name}" /><figcaption>${name}</figcaption></figure>`;
    }).join('');
    return `
      <section class="card">
        <h3>${idx + 1}. ${escapeHtml(r.stage || '-')} - ${escapeHtml(r.unitName || '-')}</h3>
        <p><strong>Status:</strong> ${escapeHtml(r.status || '-')}</p>
        <p><strong>Date:</strong> ${escapeHtml(formatDateTime(r.date))}</p>
        <p><strong>Reason:</strong> ${escapeHtml(r.reason || '-')}</p>
        <p><strong>Comment:</strong> ${escapeHtml(r.comment || '-')}</p>
        <div class="photos">${photosHtml || '<p class="muted">No photos</p>'}</div>
      </section>
    `;
  }).join('');
...
<head>
  ...
  <style>
    ...
    .header { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 16px; margin-bottom: 16px; }
    h1 { margin: 0 0 8px; font-size: 24px; color: #0f172a; }
    .meta { margin: 2px 0; font-size: 14px; color: #334155; }
    .card { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 14px; margin-bottom: 14px; }
    .card h3 { margin: 0 0 8px; color: #0f172a; }
    .card p { margin: 4px 0; font-size: 14px; }
    .photos { margin-top: 10px; display: flex; flex-wrap: wrap; gap: 10px; }
    figure { margin: 0; width: 260px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 8px; }
...
<body>
  <div class="header">
    <h1>Full Data Report</h1>
    <p class="meta"><strong>Operative:</strong> ${escapeHtml(meta.workerName)}</p>
    <p class="meta"><strong>Project:</strong> ${escapeHtml(meta.projectName)}</p>
    <p class="meta"><strong>Period:</strong> ${escapeHtml(period)}</p>
    <p class="meta"><strong>Generated:</strong> ${escapeHtml(createdAt)}</p>
  </div>
  ${items || '<p class="muted">No records found.</p>'}
  <footer>Generated by proconix.uk</footer>
</body>
</html>`;
}
...
async function generateDailyRecordInvoiceFromPeriod(req, res) {
  ...
  try {
    ...
    const fullHtmlName = `Full_data_raport_${sanitizeNamePart(stamp)}.html`;
    ...
    const fullHtml = buildDailyRecordsFullHtml(records, { workerName, projectName, fromDate, toDate });
    ...
    for (const [unitKey, unitRecords] of recordsByUnit.entries()) {
      ...
      const photosCount = unitRecords.reduce((acc, rec) => acc + (Array.isArray(rec.photos) ? rec.photos.length : 0), 0);
      unitsSummary.push({
        unitName: unitDisplayName,
        entriesCount: unitRecords.length,
        photosCount: photosCount,
      });
    }
    const indexPdf = await buildZipIndexPdf(unitsSummary, { workerName, projectName, fromDate, toDate });
    ...
    const fullHtmlAbs = path.join(GENERATED_WORKLOG_DIR, fullHtmlName);
    ...
    fs.writeFileSync(fullHtmlAbs, fullHtml, 'utf8');
