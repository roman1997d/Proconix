...
async function extractZipWithFallback(zipPath, outDir, cwd) {
  const unzipCmd = resolveExecutable([
    process.env.UNZIP_PATH,
    '/usr/bin/unzip',
    '/bin/unzip',
    'unzip',
  ]);
  try {
    await runCommand(unzipCmd, ['-o', zipPath, '-d', outDir], { cwd });
    return;
  } catch (err) {
    const msg = err && err.message ? String(err.message) : '';
    if (!/ENOENT/i.test(msg)) throw err;
  ...
  await runCommand(pyCmd, ['-c', pyScript, zipPath, outDir], { cwd });
}
...
/**
 * POST /api/platform-admin/restore
 * Alias route: POST /api/admin/restore
 * Multipart form-data: field "backup" (.zip generated by backup endpoint).
 */
